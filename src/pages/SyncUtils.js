            ...
            db.get('log').then(function(doc) {
                ...
                writeLog().then((res)=>{
                    ...
                    startSync().then(async (res) => {
                        ...
                        await identifySql().then(async (res) => {
...
function startSync(){
    return new Promise ((resolve, reject) => {
        try {
            let usuario = localStorage.getItem("macropecas")
            fetch(server+'/startSync?user='+usuario).then(r => r.json()).then(r => resolve(r)).catch(err => {reject(err) ;  throw err})
        } catch (err) {
            reject(err) ;  throw err
        }
    })
...
async function sendSql(withoutSend){
    return new Promise ((resolve, reject) => {
        try {
            if (!withoutSend){
                let usuario = localStorage.getItem("macropecas")
...
function startTransaction (withoutSend){
    return new Promise (async (resolve, reject) => {
        if (!withoutSend) {
            let usuario = localStorage.getItem("macropecas")
